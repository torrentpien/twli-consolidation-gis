...
| 2 | 財政部 | **缺 102 年 (2013) 整年 csv** | SEGIS 來源未提供，下載時即缺 |
| 3 | 財政部 | 部分年度有 V_ID 重複（如 2022 有 9 個 V_ID 對應兩個 VILLAGE 名）| 疑亂碼造成 |
| 4 | 財政部 | 15 列 V_ID 末三碼 `-999`（如 64000030-999）| 鄉鎮無法分配的彙整列，**ingest 自動過濾** |
...
- 財政部：100-111 (但 102 缺檔)，等同 12 年有效資料

joint panel 採取「主軸 LEFT JOIN」：以人口統計為主，財政部欄位在 102 與 113 為 NaN。
...
- 教育部 vs 行政部 15+ 歲人口：12 個年度 100% 完全相等；2016 微差 1、2021 微差 0.21%、2023 微差 6
...
| 財政部 | ~85,000 | 7601 SAU × 11 年 (扣 102 缺檔) |
...
| `examples/demo_tax_panel.py` | 財政部 11 年 panel + keep_if_unique 驗證 |
