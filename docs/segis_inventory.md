...
> 本文件為 SEGIS 三來源、11 主題、53 個村里級 csv 的整合說明。  
> 整併套件相關背景請見 [`README`](../README.md)（如有）與
> [`docs/plan_segis_multi_source.md`](plan_segis_multi_source.md)。

## 1. 來源結構

```
SEGIS按資料類型分/
├── 行政部/   9 主題 × 5 年 (109–113) = 45 csv
│   ├── 行政區人口統計
│   ├── 行政區人口指標                  ← 衍生指標，不直接整併
│   ├── 行政區三段年齡組性別人口統計
│   ├── 行政區五歲年齡組性別人口統計
...
│   ├── 行政區分齡兒童及少年性別人口統計
│   ├── 行政區原住民人口統計
│   ├── 行政區原住民人口指標            ← 衍生指標，不直接整併
│   └── 行政區原住民十歲年齡組性別人口統計
├── 教育部/   1 主題 × 5 年 (109–113) = 5 csv
│   └── 行政區15歲以上人口教育程度統計
└── 財政部/   1 主題 × 3 年 (109–111) = 3 csv
    └── 綜合所得稅所得總額申報統計
```
...
財政部多一個 `TAG` 欄位在最前（彙整列標籤，已自動過濾）。

- **編碼**：全部 UTF-8-BOM
- **第 2 列**：中文欄位說明（ingest 自動跳過）
- **主鍵**：`V_ID`（村里代碼，與既有 4476 件 crosswalk YAML 完全相容）
- **INFO_TIME 格式**：人口類 `113Y12M`、教育/所得稅 `113Y` 或 `111Y`
...
| 原住民人口統計 | 10 | _CNT (O/O1/O2/NON_O × 總/M/F) | sum |
| 原住民十歲年齡組 | 22 | _CNT (11 組 × M/F，無總計欄) | sum |
| 教育程度 | 9 | _CNT (博士~不識字 9 級) | sum |
| **人口指標** | 7 | RAT/CNT (性比例、戶量、密度、扶養比…) | **不直接整併，由 base recompute** |
| **原住民人口指標** | 5 | PER (比率、扶養/扶幼/扶老/老化) | **同上** |
...
### 4.1 直接 sum（167 欄位 / 9 主題）
所有 _CNT 類欄位都可直接加總。整併後的 SAU 計數欄位仍可正常加總、比較跨年。

### 4.2 衍生指標 recompute（2 主題）
...
| 2 | 行政部 原住民十歲組 | 2020 年少 423 個 V_ID（2021+ 補齊）| 2020 csv 僅列「有原住民人口」村里 |
| 3 | 行政部 各主題 | P_CNT ≠ O_CNT + NON_O_CNT（2021-2024 差 235K–340K）| NON_O 可能不含外籍/無戶籍人口 |
| 4 | 財政部 | 2022 年 9 個 V_ID 重複出現（同 V_ID 對應兩個 VILLAGE 名）| 疑亂碼造成 |
| 5 | 財政部 | 15 列 V_ID 末三碼 `-999`（如 64000030-999）| 鄉鎮無法分配的彙整列，**ingest 自動過濾** |
...
joint panel 採取「主軸 LEFT JOIN」：以人口統計為主，財政部欄位在 2023-2024 為 NaN。

### 6.2 V_ID 覆蓋不齊
個別主題某些年份 raw csv 少幾個 V_ID（見 §5）。整併後 SAU panel 列數可能略少於主軸；join 後對應欄位為 NaN。

### 6.3 跨來源加總交叉驗證
- 行政部主題之間（P_CNT == 三段/五歲/十歲組總和、性別合計、原住民組合計）：✓ 5 年全等
- 教育部 vs 行政部 15+ 歲人口：3 個年度 100% 相等，2021 微差 0.21%
- O + NON_O vs P_CNT：2021+ 起不等（見 §5 #3），非套件 bug
...
| 路徑 | 用途 |
|---|---|
| `examples/demo_segis_panel.py` | 行政部・人口統計 5 年 SAU panel（最小範例） |
| `examples/demo_education_panel.py` | 教育部 5 年 panel + 交叉驗證 |
| `examples/demo_tax_panel.py` | 財政部 3 年 panel + keep_if_unique 驗證 |
| `examples/demo_multi_source_panel.py` | **跨來源寬表 panel**（5 主題、50 欄、5 年） |
| `examples/check_segis_admin_topics.py` | 行政部 9 主題完整驗證（主題內 + 主題間 + 異常記錄） |
| `examples/demo_multiyear_panel.py` | 既有 7 年 panel（混合 data_population 107-108 + SEGIS 109-113） |
| `examples/check_panel_integrity.py` | 整併 panel 健全性檢查 |

## 8. 與既有 `docs/data_inventory.md` 的關係
